...
%{
    ...
    final LinkedList<Integer> states = new LinkedList();
...
<CLASS1> {
  ...
  [\n\b\t\r\f ]              { if (commentMode && whitespaceInClass) return consulo.language.ast.TokenType.WHITE_SPACE; yypushback(1); yybegin(CLASS2); }
...
<CLASS2> {
  ...
  " "                   { return (commentMode && whitespaceInClass) ? consulo.language.ast.TokenType.WHITE_SPACE : RegExpTT.CHARACTER; }
  [\n\b\t\r\f]          { return (commentMode && whitespaceInClass) ? consulo.language.ast.TokenType.WHITE_SPACE : RegExpTT.CTRL_CHARACTER; }
